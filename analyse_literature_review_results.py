...
fig.tight_layout()
plt.savefig('figs/lit_rev_count_proportions_over_time_by_value_group.png')
plt.show()
# %%
# grouped_time_data = molten_data.groupby([C_YEAR, C_VALUE_GROUP]).sum().reset_index()
# fig, ax = plt.subplots(figsize=(12, 6))
...
ax.grid(axis='y')
ax.set_yticks(np.arange(0, df_pre_screening[C_TITLE].max(), 20))
# ax.set_yticks(np.arange(0, df_pre_screening[C_TITLE].max(), 5), minor=True)
